...
def decode_b64(encoded, key):
    return decode(b64decode(encoded), key)


def decode(encoded, key):
    cipher = Cipher(algorithms.AES(key), modes.ECB(), default_backend())
    decryptor = cipher.decryptor()
    return decryptor.update(encoded) + decryptor.finalize()


def test_decode_b64():
    assert decode_b64(encoded, b'YELLOW SUBMARINE').startswith(b"I'm back and I'm ringin' the bell")
